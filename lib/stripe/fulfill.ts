...
import { fulfillMembershipFromSession } from "@/lib/stripe/membership-subscription";
import { walletApplyDebitForRaceEntry } from "@/lib/wallet/wallet-race-entry";

export { fulfillMembershipFromSession };
...
export async function fulfillRaceEntryFromSession(session: Stripe.Checkout.Session): Promise<{
  ...
  if (payload.kioskWalkUpMembership?.tier) {
    const end = new Date();
    end.setFullYear(end.getFullYear() + 1);
    await service
      .from("memberships")
      .update({
        tier: payload.kioskWalkUpMembership.tier,
        status: "active",
  ...
  if (session.metadata?.kiosk_walk_up === "1") {
    await kioskAutoCheckInWalkUpRunner(service, eventId, userId);
  }
