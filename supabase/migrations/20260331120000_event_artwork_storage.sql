-- Optional race artwork: nullable URL on events; files in Storage for promoters who upload.
-- Uses SECURITY DEFINER helper so storage policies can read events without RLS blocking nested checks.

-- Verify promoter (or admin) for object path "event_id/filename" — runs as definer, still checks auth.uid().
