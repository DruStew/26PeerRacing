...
import type { PublicDivision, PublicResults } from "@/lib/results-public";
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-xl border border-[#1E3A5F]/10 bg-white p-6 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#1E3A5F]/55">Checks paid</p>
          ...
          <div className="rounded-lg border border-[#E87722]/30 bg-[#E87722]/5 px-3 py-2">
            ...
            <p className="font-display mt-1 text-2xl font-bold text-[#1E3A5F]">{formatUsd(r.totalPayoutCents)}</p>
          </div>
        </div>
      ...
      {r.divisions.length > 0 || r.incentives.length > 0 ? (
        <section className="rounded-xl border border-[#1E3A5F]/10 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-xs text-[#1E3A5F]/65">
            Every finisher earns their division badge; paid places also win money.
          </p>
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-[#1E3A5F]/10">
          {runners.map((f, idx) => {
            ...
            return (
              <tr key={f.id} className={amount > 0 ? "bg-[#fff9f5]" : undefined}>
